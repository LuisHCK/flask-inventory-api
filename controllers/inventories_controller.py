...
def index():
    ...
    for inventory in Inventory.select():
        inventories.append(
            {**inventory.to_dict(), "total_products": sum(inventory.inventory_products)}
        )
...
def update_inventory_product(inventory_id, product_id):
    body = __whitelisted_params(request.json)
    inventory_product = InventoryProduct.select(
...
@jwt_required()
def delete_inventory_product(inventory_id, product_id):
    inventory_product = InventoryProduct.select(
        inventory=inventory_id, product=product_id
    )

    if inventory_product.count() > 0:
        inventory_product.delete(bulk=True)
        return {"message": "Product successfully removed from the inventory"}

    return {"message": "Product not found in this inventory"}, 404
